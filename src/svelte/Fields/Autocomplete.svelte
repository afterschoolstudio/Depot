<script>
  ...
  function safeLabelFunction(item) {
    ...
    if (aliases.length === items.length && aliases.length > 0)
      return aliases[items.findIndex((o) => o === item)];
  ...
  function prepareUserEnteredText(userEnteredText) {
    ...
    const cleanUserEnteredText = safeStringFunction(
      textCleanFunction,
      textFiltered
    );
  ...
  async function search() {
    ...
    if (
      ...
      textFiltered.length < minCharactersToSearch
    ) {
      textFiltered = "";
    }
    ...
    if (debug) {
      console.log(
        "Changed user entered text '" + text + "' into '" + textFiltered + "'"
      );
    }
    ...
    if (!searchFunction) {
      ...
      if (searchFunction.constructor.name === "AsyncGeneratorFunction") {
        for await (const chunk of searchFunction(
          textFiltered,
